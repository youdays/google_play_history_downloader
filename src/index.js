(() => {
  ...
  const isGooglePlrayHistoryPage = () => {
    return location.href.startsWith(GOOGLE_PLAY_HISTORY_URL);
  };
  ...
  const main = async () => {
    if (!isGooglePlrayHistoryPage()) {
      alert(
        "「注文履歴 - Google Play」(" +
          GOOGLE_PLAY_HISTORY_URL +
          " )で実行してください。"
      );
      return false;
    }
